# order_editor.py - Полноценный редактор постановлений с загрузкой изображений
import sys
import os
import requests
...
from PyQt6.QtGui import QIcon, QClipboard, QPixmap
from utils import get_resource_path

# Функция для загрузки изображения на imgbb.com (бесплатный хостинг)
def upload_to_imgbb(image_path, api_key='6b7a6a3c7f5e8d9c4b3a2f1e0d9c8b7a'):
    """
    Загружает изображение на imgbb.com и возвращает прямую ссылку
    """
    try:
...
class OrderEditorWindow(QMainWindow):
    def __init__(self):
        ...
        self.fields_widgets = {}
        self.header_url = ""  # Ссылка на шапку
    ...
    def init_ui(self):
        ...
        tabs.setStyleSheet("""
            ...
            QTabBar::tab { background-color: #2d2d2d; color: white; padding: 8px 20px; margin-right: 2px; }
        ...
        scroll_layout.addWidget(self.fields_container)
        ...
        scroll_layout.addLayout(items_btn_layout)

        scroll_layout.addWidget(QLabel("📋 Пункты постановления:"))

        self.items_list = QListWidget()
        self.items_list.setSelectionMode(QListWidget.SelectionMode.SingleSelection)
        scroll_layout.addWidget(self.items_list)
        ...
        preview_layout.addWidget(QLabel("📋 Предпросмотр BBCode:"))
        ...
        # Кнопки действий под предпросмотром
    ...
    def generate(self):
        try:
            ...
            elif tmpl["header_img"] and not self.header_url:
                # Если шаблон предусматривает шапку, но ссылки нет - пропускаем
                pass
    ...
    def reset_fields(self):
        self.load_template()
        self.info_label.setText("🔄 Поля сброшены")
...
class LauncherWindow(QMainWindow):
    ...
    def __init__(self, user_data):
        ...
        self.setWindowTitle("GIH - Лаунчер")
    ...
    def init_ui(self):
        ...
        # Welcome
        lbl_welcome = QLabel(f"Добро пожаловать, {self.user_data.get('Username')}!")
        lbl_welcome.setStyleSheet("font-size: 24px; font-weight: bold; color: #4facfe;")
        layout.addWidget(lbl_welcome, alignment=Qt.AlignmentFlag.AlignCenter)

        lbl_role = QLabel(f"Роль: {self.user_data.get('Role')}")
        lbl_role.setStyleSheet("font-size: 14px; color: #aaa; margin-bottom: 20px;")
        layout.addWidget(lbl_role, alignment=Qt.AlignmentFlag.AlignCenter)

        # Buttons
        btn_table = QPushButton("📂 Открыть Таблицу")
        btn_table.clicked.connect(self.open_table)
        layout.addWidget(btn_table, alignment=Qt.AlignmentFlag.AlignCenter)

        btn_orders = QPushButton("📝 Редактор Постановлений")
        btn_orders.clicked.connect(self.open_order_editor)
        layout.addWidget(btn_orders, alignment=Qt.AlignmentFlag.AlignCenter)

        btn_exit = QPushButton("🚪 Выход")
        btn_exit.setStyleSheet("background-color: #d63031;") 
    ...
    def open_order_editor(self):
        self.order_editor = OrderEditorWindow()
        self.order_editor.show()
        # Keep reference to avoid garbage collection? 
        # But we want to close launcher? 
        # If we close launcher, and it's the main window...
        # Let's hide launcher or keep it open?
        # If we open order editor, maybe we don't close launcher?
        # Or maybe we do. Let's keep it simple for now.
